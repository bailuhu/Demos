...
BOOL CMailMessage::AddMultipleRecipients(LPCTSTR szRecipients )
{
	...
	// Add Recipients
	USES_CONVERSION;
	length = strlen( T2A(szRecipients));
	char* buf = new char[ length + 1 ];	// Allocate a work area (don't touch parameter itself)
	strcpy( buf, T2A(szRecipients) );
...
BOOL CMailMessage::EncodeHeader()
{
	...
	sDate = m_tDateTime.Format( _T("%a, %d %b %y %H:%M:%S %Z") );
...
void CMailMessage::EncodeBody()
{
	...
	LPTSTR szGood = _T("\r\n..\r\n");
	int nPos;
	char* szBad_char = NULL;
	int len = WideCharToMultiByte(CP_ACP, 0, szBad, -1, NULL, 0, NULL, NULL);
	WideCharToMultiByte(CP_ACP, 0, szBad, -1, szBad_char, len, NULL, NULL);   	// 将tchar值赋给_char 
	int nBadLength = strlen( szBad_char );
	if( m_sBody.Left( 3 ) == _T(".\r\n") )
		m_sBody = _T(".") + m_sBody;
	//
