...
BOOL CSMTP::Connect()
{
	...
	char* local_host_char = NULL;
	int len = WideCharToMultiByte(CP_ACP, 0, local_host, -1, NULL, 0, NULL, NULL);
	WideCharToMultiByte(CP_ACP, 0, local_host, -1, local_host_char, len, NULL, NULL);   	// ½«tcharÖµ¸³¸ø_char 
	gethostname( local_host_char, 80 );
...
BOOL CSMTP::transmit_message(CMailMessage * msg)
{
	...
	sTemp=_T("X-Mailer: CSMTP class for MFC\r\n");
	m_wsSMTPServer.Send( (LPCTSTR)sTemp, sTemp.GetLength() );
...
BOOL CSMTP::get_response( UINT response_expected )
{
	...
	USES_CONVERSION;
	sscanf( T2A(sResponse.Left(3)), "%d", &sResponse );
